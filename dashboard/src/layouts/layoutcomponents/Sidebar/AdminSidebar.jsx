...
import { AdminMenuloop } from '.././Menuloop';
import store from '../../../common/redux/Store';
import { ADMINMENUITEMS } from '../../../common/Sidemenudata';
...
const Sidebar = ({ local_varaiable, ThemeChanger }) => {
    ...
    function closeMenuFn() {
        ...
        closeMenuRecursively(ADMINMENUITEMS);
    ...
    function setSubmenu(event, targetObject, ADMINMENUITEMS = menuitems) {
        const theme = store.getState();
    ...
    function setMenuUsingUrl(currentPath) {
        ...
        setSubmenuRecursively(ADMINMENUITEMS);
    }
    ...
    function toggleSidemenu(event, targetObject, ADMINMENUITEMS = menuitems) {
        const theme = store.getState();
        ...
        if ((theme.datanavstyle != "icon-hover" && theme.datanavstyle != "menu-hover") || (window.innerWidth < 992) || (theme.datanavlayout != "horizontal") && (theme.toggled != "icon-hover-closed" && theme.toggled != "menu-hover-closed")) {
            for (const item of ADMINMENUITEMS) {
                if (item === targetObject) {
                    ...
                    if (item.active) {
                        closeOtherMenus(ADMINMENUITEMS, item);
                    } else {
                        if (theme.dataverticalstyle == 'doublemenu') {
                            ThemeChanger({ ...theme, toggled: "double-menu-close" });
                        }
                    }
                    setAncestorsActive(ADMINMENUITEMS, item);
    ...
    function setAncestorsActive(ADMINMENUITEMS, targetObject) {
        const theme = store.getState();
        const parent = findParent(ADMINMENUITEMS, targetObject);
        if (parent) {
            parent.active = true;
            if (parent.active) {
                ThemeChanger({ ...theme, toggled: "double-menu-open" });
            ...
            setAncestorsActive(ADMINMENUITEMS, parent);
        } else {
            if (theme.dataverticalstyle == "doublemenu") {
                ThemeChanger({ ...theme, toggled: "double-menu-close" });
            }
    ...
    function closeOtherMenus(ADMINMENUITEMS, targetObject) {
        for (const item of ADMINMENUITEMS) {
            if (item !== targetObject) {
                item.active = false;
                if (item.children && item.children.length > 0) {
    ...
    function findParent(ADMINMENUITEMS, targetObject) {
        for (const item of ADMINMENUITEMS) {
            if (item.children && item.children.includes(targetObject)) {
                return item;
            }
            if (item.children && item.children.length > 0) {
                const parent = findParent(ADMINMENUITEMS = item.children, targetObject);
                if (parent) {
                    return parent;
                }
            }
                            ...
                            {ADMINMENUITEMS.map((levelone) => (
                                <Fragment key={Math.random()}>
                                    <li className={`${levelone.menutitle ? 'slide__category' : ''} 
                                        ...
                                        {/* if Sub level  */}
                                        {levelone.type === "sub" ? <AdminMenuloop ADMINMENUITEMS={levelone} level={level + 1} toggleSidemenu={toggleSidemenu} /> : ''}
                                    </li>
                                </Fragment>
                            ))}
                        </ul>
