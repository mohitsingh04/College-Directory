...
import { AgentMenuloop } from '.././Menuloop';
import store from '../../../common/redux/Store';
import { AGENTMENUITEMS } from '../../../common/Sidemenudata';
...
const Sidebar = ({ local_varaiable, ThemeChanger }) => {
    ...
    const [menuitems, setMenuitems] = useState(AGENTMENUITEMS);
    ...
    function closeMenuFn() {
        ...
        closeMenuRecursively(AGENTMENUITEMS);
    ...
    function setSubmenu(event, targetObject, AGENTMENUITEMS = menuitems) {
        const theme = store.getState();
    ...
    function setMenuUsingUrl(currentPath) {
        ...
        setSubmenuRecursively(AGENTMENUITEMS);
    }
    ...
    function toggleSidemenu(event, targetObject, AGENTMENUITEMS = menuitems) {
        const theme = store.getState();
        ...
        if ((theme.datanavstyle != "icon-hover" && theme.datanavstyle != "menu-hover") || (window.innerWidth < 992) || (theme.datanavlayout != "horizontal") && (theme.toggled != "icon-hover-closed" && theme.toggled != "menu-hover-closed")) {
            for (const item of AGENTMENUITEMS) {
                if (item === targetObject) {
                    ...
                    if (item.active) {
                        closeOtherMenus(AGENTMENUITEMS, item);
                    } else {
                        if (theme.dataverticalstyle == 'doublemenu') {
                            ThemeChanger({ ...theme, toggled: "double-menu-close" });
                        }
                    }
                    setAncestorsActive(AGENTMENUITEMS, item);
    ...
    function setAncestorsActive(AGENTMENUITEMS, targetObject) {
        const theme = store.getState();
        const parent = findParent(AGENTMENUITEMS, targetObject);
        if (parent) {
            parent.active = true;
            if (parent.active) {
                ThemeChanger({ ...theme, toggled: "double-menu-open" });
            ...
            setAncestorsActive(AGENTMENUITEMS, parent);
        } else {
            if (theme.dataverticalstyle == "doublemenu") {
                ThemeChanger({ ...theme, toggled: "double-menu-close" });
            }
    ...
    function closeOtherMenus(AGENTMENUITEMS, targetObject) {
        for (const item of AGENTMENUITEMS) {
            if (item !== targetObject) {
                item.active = false;
                if (item.children && item.children.length > 0) {
    ...
    function findParent(AGENTMENUITEMS, targetObject) {
        for (const item of AGENTMENUITEMS) {
            if (item.children && item.children.includes(targetObject)) {
                return item;
            }
            if (item.children && item.children.length > 0) {
                const parent = findParent(AGENTMENUITEMS = item.children, targetObject);
                if (parent) {
                    return parent;
                }
            }
                            ...
                            {AGENTMENUITEMS.map((levelone) => (
                                <Fragment key={Math.random()}>
                                    <li className={`${levelone.menutitle ? 'slide__category' : ''} 
                                        ...
                                        {/* if Sub level  */}
                                        {levelone.type === "sub" ? <AgentMenuloop AGENTMENUITEMS={levelone} level={level + 1} toggleSidemenu={toggleSidemenu} /> : ''}
                                    </li>
                                </Fragment>
                            ))}
                        </ul>
