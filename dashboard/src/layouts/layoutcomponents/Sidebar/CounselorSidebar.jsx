...
import { CounselorMenuloop } from '.././Menuloop';
import store from '../../../common/redux/Store';
import { COUNSELORMENUITEMS } from '../../../common/Sidemenudata';
...
const Sidebar = ({ local_varaiable, ThemeChanger }) => {
    ...
    const [menuitems, setMenuitems] = useState(COUNSELORMENUITEMS);
    ...
    function closeMenuFn() {
        ...
        closeMenuRecursively(COUNSELORMENUITEMS);
    ...
    function setSubmenu(event, targetObject, COUNSELORMENUITEMS = menuitems) {
        const theme = store.getState();
        if ((window.screen.availWidth <= 992 || theme.datanavlayout != "icon-hover") && (window.screen.availWidth <= 992 || theme.datanavlayout != "menu-hover")) {
            if (!event?.ctrlKey) {
                for (const item of COUNSELORMENUITEMS) {
    ...
    function setMenuUsingUrl(currentPath) {
        ...
        setSubmenuRecursively(COUNSELORMENUITEMS);
    }
    ...
    function toggleSidemenu(event, targetObject, COUNSELORMENUITEMS = menuitems) {
        const theme = store.getState();
        ...
        if ((theme.datanavstyle != "icon-hover" && theme.datanavstyle != "menu-hover") || (window.innerWidth < 992) || (theme.datanavlayout != "horizontal") && (theme.toggled != "icon-hover-closed" && theme.toggled != "menu-hover-closed")) {
            for (const item of COUNSELORMENUITEMS) {
                if (item === targetObject) {
                    ...
                    if (item.active) {
                        closeOtherMenus(COUNSELORMENUITEMS, item);
                    } else {
                        if (theme.dataverticalstyle == 'doublemenu') {
                            ThemeChanger({ ...theme, toggled: "double-menu-close" });
                        }
                    }
                    setAncestorsActive(COUNSELORMENUITEMS, item);
    ...
    function setAncestorsActive(COUNSELORMENUITEMS, targetObject) {
        const theme = store.getState();
        const parent = findParent(COUNSELORMENUITEMS, targetObject);
        if (parent) {
            parent.active = true;
            if (parent.active) {
                ThemeChanger({ ...theme, toggled: "double-menu-open" });
            ...
            setAncestorsActive(COUNSELORMENUITEMS, parent);
        } else {
            if (theme.dataverticalstyle == "doublemenu") {
                ThemeChanger({ ...theme, toggled: "double-menu-close" });
            }
    ...
    function closeOtherMenus(COUNSELORMENUITEMS, targetObject) {
        for (const item of COUNSELORMENUITEMS) {
            if (item !== targetObject) {
                item.active = false;
                if (item.children && item.children.length > 0) {
    ...
    function findParent(COUNSELORMENUITEMS, targetObject) {
        for (const item of COUNSELORMENUITEMS) {
            if (item.children && item.children.includes(targetObject)) {
                return item;
            }
            if (item.children && item.children.length > 0) {
                const parent = findParent(COUNSELORMENUITEMS = item.children, targetObject);
                if (parent) {
                    return parent;
                }
            }
                ...
                <SimpleBar className="main-sidebar" id="sidebar-scroll">
                    ...
                    <nav className="main-menu-container nav nav-pills flex-column sub-open">
                        ...
                        <ul className="main-menu" onClick={() => Sideclick()}>
                            {COUNSELORMENUITEMS.map((levelone) => (
                                <Fragment key={Math.random()}>
                                    <li className={`${levelone.menutitle ? 'slide__category' : ''} 
                                        ...
                                        {/* if Sub level  */}
                                        {levelone.type === "sub" ? <CounselorMenuloop COUNSELORMENUITEMS={levelone} level={level + 1} toggleSidemenu={toggleSidemenu} /> : ''}
                                    </li>
                                </Fragment>
                            ))}
                        </ul>
