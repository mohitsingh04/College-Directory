...
import {CyberPartnerMenuloop} from '.././Menuloop';
import store from '../../../common/redux/Store';
import { CYBERPARTNERMENUITEMS } from '../../../common/Sidemenudata';
...
const Sidebar = ({ local_varaiable, ThemeChanger }) => {
    ...
    const [menuitems, setMenuitems] = useState(CYBERPARTNERMENUITEMS);
    ...
    function closeMenuFn() {
        ...
        closeMenuRecursively(CYBERPARTNERMENUITEMS);
    ...
    function setSubmenu(event, targetObject, CYBERPARTNERMENUITEMS = menuitems) {
        const theme = store.getState();
        if ((window.screen.availWidth <= 992 || theme.datanavlayout != "icon-hover") && (window.screen.availWidth <= 992 || theme.datanavlayout != "menu-hover")) {
            if (!event?.ctrlKey) {
                for (const item of CYBERPARTNERMENUITEMS) {
    ...
    function setMenuUsingUrl(currentPath) {
        ...
        setSubmenuRecursively(CYBERPARTNERMENUITEMS);
    }
    ...
    function toggleSidemenu(event, targetObject, CYBERPARTNERMENUITEMS = menuitems) {
        const theme = store.getState();
        ...
        if ((theme.datanavstyle != "icon-hover" && theme.datanavstyle != "menu-hover") || (window.innerWidth < 992) || (theme.datanavlayout != "horizontal") && (theme.toggled != "icon-hover-closed" && theme.toggled != "menu-hover-closed")) {
            for (const item of CYBERPARTNERMENUITEMS) {
                if (item === targetObject) {
                    ...
                    if (item.active) {
                        closeOtherMenus(CYBERPARTNERMENUITEMS, item);
                    } else {
                        if (theme.dataverticalstyle == 'doublemenu') {
                            ThemeChanger({ ...theme, toggled: "double-menu-close" });
                        }
                    }
                    setAncestorsActive(CYBERPARTNERMENUITEMS, item);
    ...
    function setAncestorsActive(CYBERPARTNERMENUITEMS, targetObject) {
        const theme = store.getState();
        const parent = findParent(CYBERPARTNERMENUITEMS, targetObject);
        if (parent) {
            parent.active = true;
            if (parent.active) {
                ThemeChanger({ ...theme, toggled: "double-menu-open" });
            ...
            setAncestorsActive(CYBERPARTNERMENUITEMS, parent);
        } else {
            if (theme.dataverticalstyle == "doublemenu") {
                ThemeChanger({ ...theme, toggled: "double-menu-close" });
            }
    ...
    function closeOtherMenus(CYBERPARTNERMENUITEMS, targetObject) {
        for (const item of CYBERPARTNERMENUITEMS) {
            if (item !== targetObject) {
                item.active = false;
                if (item.children && item.children.length > 0) {
    ...
    function findParent(CYBERPARTNERMENUITEMS, targetObject) {
        for (const item of CYBERPARTNERMENUITEMS) {
            if (item.children && item.children.includes(targetObject)) {
                return item;
            }
            if (item.children && item.children.length > 0) {
                const parent = findParent(CYBERPARTNERMENUITEMS = item.children, targetObject);
                if (parent) {
                    return parent;
                }
            }
                ...
                <SimpleBar className="main-sidebar" id="sidebar-scroll">
                    ...
                    <nav className="main-menu-container nav nav-pills flex-column sub-open">
                        ...
                        <ul className="main-menu" onClick={() => Sideclick()}>
                            {CYBERPARTNERMENUITEMS.map((levelone) => (
                                <Fragment key={Math.random()}>
                                    <li className={`${levelone.menutitle ? 'slide__category' : ''} 
                                        ...
                                        {/* if Sub level  */}
                                        {levelone.type === "sub" ? <CyberPartnerMenuloop CYBERPARTNERMENUITEMS={levelone} level={level + 1} toggleSidemenu={toggleSidemenu} /> : ''}
                                    </li>
                                </Fragment>
                            ))}
                        </ul>
