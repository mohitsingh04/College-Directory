...
import { EditorMenuloop } from '.././Menuloop';
import store from '../../../common/redux/Store';
import { EDITORMENUITEMS } from '../../../common/Sidemenudata';
...
const Sidebar = ({ local_varaiable, ThemeChanger }) => {
    ...
    const [menuitems, setMenuitems] = useState(EDITORMENUITEMS);
    ...
    function closeMenuFn() {
        ...
        closeMenuRecursively(EDITORMENUITEMS);
    ...
    function setSubmenu(event, targetObject, EDITORMENUITEMS = menuitems) {
        const theme = store.getState();
    ...
    function setMenuUsingUrl(currentPath) {
        ...
        setSubmenuRecursively(EDITORMENUITEMS);
    }
    ...
    function toggleSidemenu(event, targetObject, EDITORMENUITEMS = menuitems) {
        const theme = store.getState();
        ...
        if ((theme.datanavstyle != "icon-hover" && theme.datanavstyle != "menu-hover") || (window.innerWidth < 992) || (theme.datanavlayout != "horizontal") && (theme.toggled != "icon-hover-closed" && theme.toggled != "menu-hover-closed")) {
            for (const item of EDITORMENUITEMS) {
                if (item === targetObject) {
                    ...
                    if (item.active) {
                        closeOtherMenus(EDITORMENUITEMS, item);
                    } else {
                        if (theme.dataverticalstyle == 'doublemenu') {
                            ThemeChanger({ ...theme, toggled: "double-menu-close" });
                        }
                    }
                    setAncestorsActive(EDITORMENUITEMS, item);
    ...
    function setAncestorsActive(EDITORMENUITEMS, targetObject) {
        const theme = store.getState();
        const parent = findParent(EDITORMENUITEMS, targetObject);
        if (parent) {
            parent.active = true;
            if (parent.active) {
                ThemeChanger({ ...theme, toggled: "double-menu-open" });
            ...
            setAncestorsActive(EDITORMENUITEMS, parent);
        } else {
            if (theme.dataverticalstyle == "doublemenu") {
                ThemeChanger({ ...theme, toggled: "double-menu-close" });
            }
    ...
    function closeOtherMenus(EDITORMENUITEMS, targetObject) {
        for (const item of EDITORMENUITEMS) {
            if (item !== targetObject) {
                item.active = false;
                if (item.children && item.children.length > 0) {
    ...
    function findParent(EDITORMENUITEMS, targetObject) {
        for (const item of EDITORMENUITEMS) {
            if (item.children && item.children.includes(targetObject)) {
                return item;
            }
            if (item.children && item.children.length > 0) {
                const parent = findParent(EDITORMENUITEMS = item.children, targetObject);
                if (parent) {
                    return parent;
                }
            }
                            ...
                            {EDITORMENUITEMS.map((levelone) => (
                                <Fragment key={Math.random()}>
                                    <li className={`${levelone.menutitle ? 'slide__category' : ''} 
                                        ...
                                        {/* if Sub level  */}
                                        {levelone.type === "sub" ? <EditorMenuloop EDITORMENUITEMS={levelone} level={level + 1} toggleSidemenu={toggleSidemenu} /> : ''}
                                    </li>
                                </Fragment>
                            ))}
                        </ul>
