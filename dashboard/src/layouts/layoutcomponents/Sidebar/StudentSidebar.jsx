...
import {StudentMenuloop} from '.././Menuloop';
import store from '../../../common/redux/Store';
import { STUDENTMENUITEMS } from '../../../common/Sidemenudata';
...
const Sidebar = ({ local_varaiable, ThemeChanger }) => {
    ...
    function closeMenuFn() {
        ...
        closeMenuRecursively(STUDENTMENUITEMS);
    ...
    function setSubmenu(event, targetObject, STUDENTMENUITEMS = menuitems) {
        const theme = store.getState();
    ...
    function setMenuUsingUrl(currentPath) {
        ...
        setSubmenuRecursively(STUDENTMENUITEMS);
    }
    ...
    function toggleSidemenu(event, targetObject, STUDENTMENUITEMS = menuitems) {
        const theme = store.getState();
        ...
        if ((theme.datanavstyle != "icon-hover" && theme.datanavstyle != "menu-hover") || (window.innerWidth < 992) || (theme.datanavlayout != "horizontal") && (theme.toggled != "icon-hover-closed" && theme.toggled != "menu-hover-closed")) {
            for (const item of STUDENTMENUITEMS) {
                if (item === targetObject) {
                    ...
                    if (item.active) {
                        closeOtherMenus(STUDENTMENUITEMS, item);
                    } else {
                        if (theme.dataverticalstyle == 'doublemenu') {
                            ThemeChanger({ ...theme, toggled: "double-menu-close" });
                        }
                    }
                    setAncestorsActive(STUDENTMENUITEMS, item);
    ...
    function setAncestorsActive(STUDENTMENUITEMS, targetObject) {
        const theme = store.getState();
        const parent = findParent(STUDENTMENUITEMS, targetObject);
        if (parent) {
            parent.active = true;
            if (parent.active) {
                ThemeChanger({ ...theme, toggled: "double-menu-open" });
            ...
            setAncestorsActive(STUDENTMENUITEMS, parent);
        } else {
            if (theme.dataverticalstyle == "doublemenu") {
                ThemeChanger({ ...theme, toggled: "double-menu-close" });
            }
    ...
    function closeOtherMenus(STUDENTMENUITEMS, targetObject) {
        for (const item of STUDENTMENUITEMS) {
            if (item !== targetObject) {
                item.active = false;
                if (item.children && item.children.length > 0) {
    ...
    function findParent(STUDENTMENUITEMS, targetObject) {
        for (const item of STUDENTMENUITEMS) {
            if (item.children && item.children.includes(targetObject)) {
                return item;
            }
            if (item.children && item.children.length > 0) {
                const parent = findParent(STUDENTMENUITEMS = item.children, targetObject);
                if (parent) {
                    return parent;
                }
            }
                            ...
                            {STUDENTMENUITEMS.map((levelone) => (
                                <Fragment key={Math.random()}>
                                    <li className={`${levelone.menutitle ? 'slide__category' : ''} 
                                        ...
                                        {/* if Sub level  */}
                                        {levelone.type === "sub" ? <StudentMenuloop STUDENTMENUITEMS={levelone} level={level + 1} toggleSidemenu={toggleSidemenu} /> : ''}
                                    </li>
                                </Fragment>
                            ))}
                        </ul>
