...
import { SuperAdminMenuloop } from '.././Menuloop';
...
const Sidebar = ({ local_varaiable, ThemeChanger }) => {
    ...
    function closeMenuFn() {
        ...
        closeMenuRecursively(SUPERADMINMENUITEMS);
    ...
    function setSubmenu(event, targetObject, SUPERADMINMENUITEMS = menuitems) {
        const theme = store.getState();
    ...
    function setMenuUsingUrl(currentPath) {
        ...
        setSubmenuRecursively(SUPERADMINMENUITEMS);
    }
    ...
    function toggleSidemenu(event, targetObject, SUPERADMINMENUITEMS = menuitems) {
        const theme = store.getState();
        ...
        if ((theme.datanavstyle != "icon-hover" && theme.datanavstyle != "menu-hover") || (window.innerWidth < 992) || (theme.datanavlayout != "horizontal") && (theme.toggled != "icon-hover-closed" && theme.toggled != "menu-hover-closed")) {
            for (const item of SUPERADMINMENUITEMS) {
                if (item === targetObject) {
                    ...
                    if (item.active) {
                        closeOtherMenus(SUPERADMINMENUITEMS, item);
                    } else {
                        if (theme.dataverticalstyle == 'doublemenu') {
                            ThemeChanger({ ...theme, toggled: "double-menu-close" });
                        }
                    }
                    setAncestorsActive(SUPERADMINMENUITEMS, item);
    ...
    function setAncestorsActive(SUPERADMINMENUITEMS, targetObject) {
        const theme = store.getState();
        const parent = findParent(SUPERADMINMENUITEMS, targetObject);
        if (parent) {
            parent.active = true;
            if (parent.active) {
                ThemeChanger({ ...theme, toggled: "double-menu-open" });
            ...
            setAncestorsActive(SUPERADMINMENUITEMS, parent);
        } else {
            if (theme.dataverticalstyle == "doublemenu") {
                ThemeChanger({ ...theme, toggled: "double-menu-close" });
            }
    ...
    function closeOtherMenus(SUPERADMINMENUITEMS, targetObject) {
        for (const item of SUPERADMINMENUITEMS) {
            if (item !== targetObject) {
                item.active = false;
                if (item.children && item.children.length > 0) {
    ...
    function findParent(SUPERADMINMENUITEMS, targetObject) {
        for (const item of SUPERADMINMENUITEMS) {
            if (item.children && item.children.includes(targetObject)) {
                return item;
            }
            if (item.children && item.children.length > 0) {
                const parent = findParent(SUPERADMINMENUITEMS = item.children, targetObject);
                if (parent) {
                    return parent;
                }
            }
                ...
                <SimpleBar className="main-sidebar" id="sidebar-scroll">
                    ...
                    <nav className="main-menu-container nav nav-pills flex-column sub-open">
                        ...
                        <ul className="main-menu" onClick={() => Sideclick()}>
                            {SUPERADMINMENUITEMS.map((levelone) => (
                                <Fragment key={Math.random()}>
                                    <li className={`${levelone.menutitle ? 'slide__category' : ''} 
                                        ...
                                        {/* if Sub level  */}
                                        {levelone.type === "sub" ? <SuperAdminMenuloop SUPERADMINMENUITEMS={levelone} level={level + 1} toggleSidemenu={toggleSidemenu} /> : ''}
                                    </li>
                                </Fragment>
                            ))}
                        </ul>
