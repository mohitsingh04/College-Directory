			...
			try {
				...
				const merged = propertyList.map((property: any) => {
					const location = locationList.find(
						(loc: any) => loc.propertyId === property.uniqueId
					);
					return {
						...property,
							...
							<div className="py-4 flex items-center gap-4">
								<div className="flex-1 flex items-center gap-3 bg-gray-100 px-4 py-3 rounded-lg">
									...
									<input
										...
										onChange={(e) =>
											setSearch(e.target.value.trimStart().replace(/\s+/g, " "))
										}
										autoFocus
									/>
								</div>
								...
								{search ? (
									...
									<>
										{search.length >= 3 ? (
											<>
												{filteredData.map((property) => (
													<div
														...
														className="flex items-center gap-4 px-4 py-3 hover:bg-gray-200 rounded-lg"
													>
														<img
														...
														<div className="flex-1">
															<h3 className="text-lg font-medium text-gray-900">
																<Link
																	...
																	href={`/${property?.property_type?.toLowerCase()}/${
																		property?.uniqueId
																	}/${property?.property_name
																		?.replace(/\s+/g, "-")
																		.toLowerCase()}/${property?.location?.city
																		?.replace(/\s+/g, "-")
																		.toLowerCase()}`}
											...
											<>
												<h4 className="text-purple-600 mb-3">
